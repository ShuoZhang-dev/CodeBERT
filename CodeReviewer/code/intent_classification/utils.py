...
class SimpleGenDataset(TextDataset):
    def __init__(self, tokenizer, pool, args, file_path, samplenum=-1):
        ...
        if os.path.exists(savep):
            ...
            self.feats = [self.convert_examples_to_features((dic, tokenizer, args)) for dic in tqdm(data)]
            torch.save(self.feats, savep)
    ...
    def convert_examples_to_features(self, item):
        ...
        target_ids = []
        if args.intent:
            msg = self.encode_remove(tokenizer, f"INTENT: {dic['cmt_label']}", args)
...
class SimpleIntClsDataset(TextDataset):
    def __init__(self, tokenizer, pool, args, file_path, intent_dict):
        self.tokenizer = tokenizer
        if isinstance(tokenizer, MyTokenizer):
            tokenizer_type = "mytok"
        elif isinstance(tokenizer, T5Tokenizer):
            ...
            tokenizer_type = "unk"
        savep = file_path.replace(".jsonl", tokenizer_type + ".simpintexps")
    ...
    def convert_examples_to_features(self, item):
        ...
        return ClsFeatures(dic["idx"], source_ids, self.intent_dict[dic["cmt_label"].strip()])

class SimpleIntJointDataset(TextDataset):
    def __init__(self, tokenizer, pool, args, file_path, intent_dict):
        self.tokenizer = tokenizer
        if isinstance(tokenizer, MyTokenizer):
            tokenizer_type = "mytok"
            ...
            tokenizer_type = "unk"
        savep = file_path.replace(".jsonl", tokenizer_type + ".simpjointexps")
        self.intent_dict = intent_dict
        if os.path.exists(savep):
            logger.info("Loading examples from {}".format(savep))
            self.feats = torch.load(savep)
    ...
    def convert_examples_to_features(self, item):
        ...
        source_ids = self.encode_remove(tokenizer, inputstr, args)
        target_ids = []
        msg = self.encode_remove(tokenizer, f"INTENT: {dic['cmt_label'].strip()}", args)
        target_ids.extend(msg)
        target_ids.append(tokenizer.msg_id)
        msg = self.encode_remove(tokenizer, dic["msg"], args)
        target_ids.extend(msg)
        ...
        return ReviewFeatures(dic["idx"], source_ids, self.intent_dict[dic["cmt_label"].strip()], target_ids, type="genmsg")
...
def build_intent_dict(train_file_path, dev_file_path):
    intent_freq = {}
    for line in open(train_file_path):
        intent = json.loads(line)['cmt_label'].strip()
        if intent not in intent_freq:
            intent_freq[intent] = 1
        else:
            intent_freq[intent] += 1
    for line in open(dev_file_path):
        intent = json.loads(line)['cmt_label'].strip()
        if intent not in intent_freq:
            intent_freq[intent] = 1
        else:
    ...
    sorted_intents = [intent for intent, freq in sorted(intent_freq.items(), key=lambda item: item[1], reverse=True)]
    num_cls = len(sorted_intents)
    intent_dict = {}
    for i in range(num_cls):
        intent_dict[sorted_intents[i]] = i
    with open("intent_dict.json", 'w') as fp:
        json.dump(intent_dict, fp)
    return num_cls


def calculate_topk_precision(gold, pred_scores, k):
    # note: index must be zero-based!
    assert len(gold) == pred_scores.shape[0]
    total_count = len(gold)
    in_topk_count = 0
    for i in range(total_count):
        scores = pred_scores[i]
        topk_indices = sorted(range(len(scores)), key=lambda j: scores[j], reverse=True)[:k]
        if gold[i] in topk_indices:
            in_topk_count += 1
    return in_topk_count * 1.0 / total_count


def calculate_non_others_top1_precision(gold, pred):
    """ top1 precision for model's non-others prediction. """
    non_others_pred, non_others_gold = [], []
    for i in range(len(pred)):
        if pred[i] == 0:
            continue
        non_others_pred.append(pred[i])
        non_others_gold.append(gold[i])
    return accuracy_score(non_others_gold, non_others_pred)


def calculate_metrics_based_on_confidence_threshold(gold, all_scores, intent_dict, threshold=None, others_class_index=0):
    ...
    for i in range(len(gold)):
        ...
        if threshold is None:
            ...
            if highest_score >= threshold:
                ...
                pred.append(others_class_index)
...
def build_inverse_intent_dict(intent_dict):
    inverse_dict = {}
    for intent, index in intent_dict.items():
        inverse_dict[int(index)] = intent
    return inverse_dict


def match_and_add_predictions_to_test_data(args, data_idx, pred, test_data_path, output_file_path):
    intent_dict = json.load(open(args.intent_dict_file))
    inverse_intent_dict = build_inverse_intent_dict(intent_dict)
    
    assert len(data_idx) == len(pred)
    idx_to_pred_dict = {}
    for i in range(len(data_idx)):
        idx_to_pred_dict[data_idx[i]] = inverse_intent_dict[pred[i]]

    idx = 0
    with ExitStack() as stack:
        test_data = stack.enter_context(open(test_data_path, 'r', encoding="utf-8"))
        add_pred_data = stack.enter_context(open(output_file_path, 'w', encoding="utf-8"))

        for line in test_data:
            dp_dict = json.loads(line)
            dp_dict['predicted_cmt_label'] = idx_to_pred_dict[idx]
            add_pred_data.write(json.dumps(dp_dict))
            add_pred_data.write("\n")
            idx += 1
    logger.info("Outputted predictions to {}".format(output_file_path))
