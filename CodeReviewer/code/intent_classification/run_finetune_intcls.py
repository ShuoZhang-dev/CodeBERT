...
from utils import SimpleIntClsDataset, build_intent_dict
...
def main(args):
    ...
    global_step = args.load_steps
    start_epoch = int(args.load_steps // args.train_steps * args.train_epochs) + 1
    epsilon = float(args.train_steps - args.load_steps) / args.train_steps
    optimizer = AdamW(
        optimizer_grouped_parameters, lr=args.learning_rate*epsilon, eps=args.adam_epsilon
    )
    args.warmup_steps = int(args.train_steps // args.train_epochs)
    if args.load_steps > args.warmup_steps:
        args.warmup_steps = 0
    scheduler = get_linear_schedule_with_warmup(
        optimizer,
        num_warmup_steps=args.warmup_steps,
        num_training_steps=(args.train_steps - args.load_steps),
    )
    ...
    for epoch in range(start_epoch, args.train_epochs + 1):
        ...
        for _, sampler, train_dataloader in get_loaders(train_files, args, tokenizer, pool):        # WARNING: this is an iterator, to save memory
            ...
            for step, examples in enumerate(train_dataloader, 1):
                ...
                if nb_tr_steps % args.gradient_accumulation_steps == 0:
                    ...
                    if args.global_rank == 0 and global_step % args.log_steps == 0:
                        ...
                        logger.info(
                            "step {}/{}: Learning rate {}, Train loss {}".format(
                                global_step,
                                args.train_steps,
...
if __name__ == "__main__":
    ...
    num_of_classes = build_intent_dict(args.train_filename, args.dev_filename)
    logger.info(f"Number of intent classes: {num_of_classes}")
    if num_of_classes != args.num_cls:
        args.num_cls = num_of_classes

    main(args)
    logger.info("Training finished.")
    # torch.multiprocessing.spawn(main, args=(args,), nprocs=torch.cuda.device_count())

    os.system("kill $(ps aux | grep run_finetune_intcls.py | grep -v grep | awk '{print $2}') ")
