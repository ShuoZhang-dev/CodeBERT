...
python -m torch.distributed.launch --nproc_per_node ${PER_NODE_GPU} --node_rank=${RANK} --nnodes=${NODES} --master_addr=${MASTER_HOST} --master_port=${MASTER_PORT} ../run_finetune_msg.py  \
  ...
  --output_dir ${mnt_dir}/CodeReviewer/finetuned_model/msg \
  --train_filename ${mnt_dir}/CodeReviewer/data/Comment_Generation/msg-train.jsonl \
  --dev_filename ${mnt_dir}/CodeReviewer/data/Comment_Generation/msg-valid.jsonl \
